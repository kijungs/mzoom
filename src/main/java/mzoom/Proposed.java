...
public class Proposed {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Tensor tensor = TensorMethods.importSparseTensor(input, dimension);
        System.out.println();
        System.out.println("running the algorithm...");
        System.out.println();
        run(tensor, output, blockNum, lower, upper, densityMeasure);
    }

    private static void printError() {
        System.err.println("Usage: run.sh input_path output_path dimension density_measure num_of_blocks lower_bound upper_bound");
    ...
    public static void run(Tensor tensor, String output, final int blockNum, final int lower, final int upper, DensityMeasure densityMeasure) throws IOException {
        ...
        measure.initialize(tensor);

        final List<Set<Integer>[]> listOfAttributeToValues = new LinkedList();
        double bestAccuracy = 0;
        for(int i = 0; i < blockNum; i++) {
            Set<Integer>[] attributeToValues = findOneBlock(tensor, lower, upper, densityMeasure);
            bestAccuracy = Math.max(bestAccuracy, removeAndEvaluateBlock((i+1), tensor, attributeToValues, oriTensor, measure));
            listOfAttributeToValues.add(attributeToValues);
        }
     ...
     * @throws IOException
     */
    private static Set<Integer>[] findOneBlock(Tensor tensor, int lower, int upper, DensityMeasure densityMeasure) throws IOException {
        ...
        final int[] iterToAttribute = new int[sumOfCardinalities];
        final int[] iterToValue = new int[sumOfCardinalities];
        IDensityMeasure measure = null;
        if(densityMeasure == DensityMeasure.Suspiciousness)
            measure = new Suspiciousness();
        else if(densityMeasure == DensityMeasure.Arithmetic)
        ...
        int maxIters = 0;
        double maxDensityAmongIters = measure.initialize(tensor);
        ...
        for (int i = 0; i < sumOfCardinalities; i++) {
            ...
            for (int attribute = 0; attribute < dimension; attribute++) {
                ...
                if (pair != null) {
                    double tempDensity = measure.ifRemoved(attribute, pair.getValue());
            ...
            double density = measure.remove(maxAttribute, pair.getValue());
            if (satisfy(sumOfCardinalities-i-1, lower, upper) && density > maxDensityAmongIters) {
                maxDensityAmongIters = density;
                maxIters = i + 1;
            }
            iterToAttribute[i] = maxAttribute;
            iterToValue[i] = valueToRemove;
        ...
        final Set<Integer>[] attributToValues = new Set[dimension];
        for (int dim = 0; dim < dimension; dim++) {
            attributToValues[dim] = new HashSet<Integer>();
        }
        for (int iter = maxIters; iter < sumOfCardinalities; iter++){
            int attribute = iterToAttribute[iter];
            int value = iterToValue[iter];
            attributToValues[attribute].add(value);
        }

        return attributToValues;
    }
    ...
    private static IMinHeap[] createHeaps(final Tensor tensor) {
        int[][] mass = TensorMethods.mass(tensor);
     ...
     * @param block
     * @param attributeToValues
     * @param oriTensor
     * @param measure
     * @return
     */
    private static double removeAndEvaluateBlock(int blockIndex, final Tensor block, final Set<Integer>[] attributeToValues, final Tensor oriTensor, final IDensityMeasure measure) {
        final boolean[][] attributeToValuesToRemove = new boolean[block.dimension][];
        for(int dim = 0; dim < block.dimension; dim++) {
            attributeToValuesToRemove[dim] = new boolean[block.cardinalities[dim]];
            for(int index : attributeToValues[dim]) {
                attributeToValuesToRemove[dim][index] = true;
            }
        }
        final int[][] attributes = block.attributes;
        final int[] measureValues = block.measureValues;
        final int[] oriValues = oriTensor.measureValues;
        long mass = 0;
        for(int i=0; i<block.omega; i++) {
            ...
            if(removed) {
                block.mass -= measureValues[i];
                measureValues[i] = 0; //remove tuple
                mass += oriValues[i];
            }
        }

        int[] carinalitiesOfBlock = new int[block.dimension];
        System.out.println("Block: " + blockIndex);
        System.out.print("Volume: ");
        for(int dim = 0; dim < block.dimension; dim++) {
            carinalitiesOfBlock[dim] = attributeToValues[dim].size();
            System.out.print(carinalitiesOfBlock[dim]);
            if(dim < block.dimension - 1) {
                System.out.print(" X ");
            }
        }
        System.out.println();
        double density = measure.density(mass, carinalitiesOfBlock);
